...
class QuadPackGeometry extends Geometry
{
     ...
     * @return Unique quad int ID.
     */
    public function add_xywh(_frame : TextureAtlasFrame, _x : Float, _y : Float, w : Float, _h : Float, _color : Color, _flipX : Bool = false, _flipY : Bool = false) : Int
    {
        var id   = Hash.uniqueHash();
        var quad = new PackedQuad(id, [
            new Vertex( new Vector(_x    , _y     ), _color.clone(), new Vector(0, 0) ),
            new Vertex( new Vector(_x + w, _y     ), _color.clone(), new Vector(0, 0) ),
            new Vertex( new Vector(_x + w, _y + _h), _color.clone(), new Vector(0, 0) ),
