	...
	public function pack(_ctx : ParcelContext, _asset : Asset)
	{
		...
		return switch _ctx.gpuApi
		{
			case 'mock', 'ogl3':
				...
				new AssetRequest(_asset.id, new ProducedShader(vertReflection, fragReflection, vertGlslPath, fragGlslPath), NoPacking);
			case 'd3d11':
				...
				switch getFxc()
				{
					case Some(fxc):
						...
						new AssetRequest(_asset.id, new ProducedShader(vertReflection, fragReflection, vertDxbcPath, fragDxbcPath), NoPacking);
					case None:
